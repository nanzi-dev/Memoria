...
def test_catch_up_runs_once_and_advances_beyond_world_now(monkeypatch):
    ...
    completion = commits[0]["schedule_completion"]
    assert datetime.fromisoformat(completion["next_run_at"]) > world_now
    assert completion["missed_count"] == 24
    assert completion["next_due_real_at"] is not None
    assert completion["lease_owner"].startswith("scheduler:")
...
def test_catch_up_replay_limit_executes_bounded_runs_and_counts_misses(monkeypatch):
    ...
    assert len({commit["execution_key"] for commit in commits}) == 3
    assert commits[0]["schedule_completion"] is None
    assert commits[1]["schedule_completion"] is None
    completion = commits[2]["schedule_completion"]
    assert completion["missed_count"] == 6
    assert completion["next_run_at"] == (first_due + timedelta(minutes=25)).isoformat()
...
def test_deduplicated_batch_can_atomically_complete_schedule():
    player_id = _create_user("deduplicated_schedule")
    character_id = "npc_luo_xiaohei"
    event_id = f"deduplicated_{uuid.uuid4().hex[:8]}"
    execution_key = f"schedule:{event_id}:2026-07-14T12:00:00+00:00"
    scheduled_for = "2026-07-14T12:00:00+00:00"
    ...
    assert repository.claim_event_schedule(
        ...
        character_id,
        player_id,
        lease_owner="deduplicated-worker",
        lease_expires_at="2026-07-14T12:02:00+00:00",
        real_now_iso=scheduled_for,
        expected_next_run_at=scheduled_for,
        expected_next_due_real_at=scheduled_for,
    )
    first = repository.commit_event_execution_batch(
        player_id=player_id,
        execution_key=execution_key,
        trigger_source="schedule",
        results_data="[]",
    ...
    replay = repository.commit_event_execution_batch(
        player_id=player_id,
        execution_key=execution_key,
        trigger_source="schedule",
        results_data="[]",
        ...
        schedule_completion={
            ...
            "character_id": character_id,
            "lease_owner": "deduplicated-worker",
            "last_checked_at": "2026-07-14T12:04:00+00:00",
            "last_run_at": scheduled_for,
            "next_run_at": "2026-07-14T12:05:00+00:00",
            "next_due_real_at": "2026-07-14T12:01:00+00:00",
            "missed_count": 7,
        },
    )
    ...
    assert schedule["next_run_at"] == "2026-07-14T12:05:00+00:00"
    assert schedule["next_due_real_at"] == "2026-07-14T12:01:00+00:00"
    assert schedule["missed_count"] == 7
