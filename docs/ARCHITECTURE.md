...
事件系统异常只会记录日志，不会阻断对话状态、短期消息和长期记忆的持久化。对话/上下文检测触发的 once 与 cooldown 事件会先竞争 `event_trigger_guard`，成功执行时再在效果事务内消费 claim 并更新时间；并发请求只有一个可以提交副作用。
...
自主或事件驱动的群聊脉冲先在内存中规划回复，再由 `commit_group_dialogue_pulse()` 在一个事务内写入全部角色消息、修正临时回复引用、更新关系与参与者统计、完成脉冲状态并创建未读通知。任一步失败都会整体回滚；长期记忆提取在事务提交后执行，失败只记录日志，不会制造“消息已写入但脉冲/通知未完成”的半状态。
