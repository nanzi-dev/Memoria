...
多角色 Prompt 会把个人长期记忆、角色间共享记忆和 `group_memory` 群体记忆整合为上下文。当前实现不会在每个玩家轮次写入群体记忆；群聊结束且有效消息数大于 6 条、摘要模型返回非空内容时，系统会将整场对话摘要写入 `session_summary`，并同步保存到 `group_memory` 供后续群聊召回。
...
7. **完整索引覆盖** — 17 个精心设计的索引确保所有常用查询路径为 O(log n)
