...
**当前进度：** 测试集合扩展到 464 tests；记忆去重、重试、对话轮次幂等、认证限流、健康检查、懒加载、会话恢复、用户资源隔离、关系图谱修订过滤、知识文档恢复、PostgreSQL 兼容和 API/安全回归测试已上线。

- [x] 单元测试覆盖（当前 464 tests，覆盖 schema、repository、编排器、事件、API、安全、知识库、语音、向量存储、世界时钟、开发者体验和 PostgreSQL 兼容）
...
当前 `pytest --collect-only -q` 收集 464 个测试。覆盖范围包括角色与事件模型、Repository、单聊/群聊编排、关系图谱、知识库 API 与文档处理恢复、语音、安全边界、向量存储、世界时钟、系统端点、开发者体验和 PostgreSQL 兼容。总数以测试收集结果为准；前端另有 API 客户端与世界时钟状态、格式化测试。
...
| v0.4 | 5 | 质量与稳定性（去重/恢复/认证限流/用户隔离/关系图谱修订过滤/PostgreSQL 兼容） | ✅ |
