...
当前实现以关系图谱为角色间关系的最高优先级来源。通过关系 API 或前端图谱创建、更新、删除关系时，系统会写入 `character_relationship_revision` 修订时间；多角色生成和主动互动会按这个时间过滤旧的关系相关长期记忆、角色间共享记忆和群体记忆，但普通玩家事实、共同经历和世界事实会保留。群聊原始历史和结束摘要提取仍按修订时间截止，避免旧关系被重新写回。
