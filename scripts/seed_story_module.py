#!/usr/bin/env python3
"""Load, seed, and reset a story module in a Memoria database."""
...
def load_story_module(module_root: Path) -> dict[str, Any]:
    ...
    if "groups" not in manifest:
        if "group" not in manifest:
            raise ValueError("故事模块清单必须提供 groups")
        manifest["groups"] = [manifest["group"]]
    groups = manifest["groups"]
    if not groups:
        raise ValueError("故事模块清单必须至少提供一个群聊")
    session_ids = [group["session_id"] for group in groups]
    thread_ids = [group["thread_id"] for group in groups]
    if len(session_ids) != len(set(session_ids)):
        raise ValueError("故事模块群聊 session_id 必须唯一")
...
def _ensure_demo_user(
    ...
    *,
    ...
    if collision and collision.get("username") != username:
        raise RuntimeError(f"{module_label} 演示账户 ID 已被其他用户占用: {user_id}")
    ...
    if not created:
        raise RuntimeError(f"创建 {module_label} 演示账户失败")
    return created, True
...
def _ensure_group_session(
    ...
    *,
    ...
    if not success:
        raise RuntimeError(f"创建 {module_label} 群聊失败: {group['name']}")
...
def _find_or_create_base(
    ...
    *,
    ...
    for knowledge_base in matches:
        ...
        if not any(document["source_type"] == source for document in documents):
            continue
...
def _seed_document(
    ...
    *,
    ...
    matches = [
        ...
        and document["source_type"] == source
    ]
    reusable = next(
        (
            document
...
def _seed_knowledge(
    ...
    *,
    ...
    for definition in module["manifest"]["knowledge_bases"]:
        knowledge_base, existed = _find_or_create_base(
            owner_user_id,
            definition,
            source=source,
        )
        ...
        if existed:
            bindings = [
                *repository.list_knowledge_bindings(
                    owner_user_id,
                    knowledge_base["knowledge_base_id"],
                ),
                *bindings,
    ...
    *,
    ...
    for knowledge_base in repository.list_knowledge_bases(owner_user_id):
        ...
        unrelated_bindings = [
            ...
            if (
                binding["target_type"],
                binding.get("target_id") or "",
            )
            not in declared_bindings
        ]
        if not unrelated_documents and not unrelated_bindings:
    ...
    owner_group_sessions = {
        ...
        and session.get("group_thread_id") in declared_thread_ids
    }
    for session in owner_group_sessions.values():
        repository.end_session(session["session_id"])


def seed_story_module(
    module_root: Path,
    *,
    ...
    module_label = manifest.get("title") or module_id
    source = f"{module_id}-demo"
    repository.init_db()
    user, created_user = _ensure_demo_user(
        username,
        password,
    ...
    _seed_characters(owner_user_id, module["cards"], source=source)
    _seed_relationships(
        owner_user_id,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--reset-module",
        action="store_true",
        help="先清理当前演示用户下清单明确列出的模块数据，再重新播种。",
    )
    return parser


def _module_label(module_root: Path) -> str:
    try:
        manifest = _read_json(Path(module_root) / "manifest.json")
    except Exception:
        return str(module_root)
    title = manifest.get("title")
    module_id = manifest.get("module_id")
    if title and module_id:
        return f"{title} ({module_id})"
    return title or module_id or str(module_root)
...
def main() -> int:
    ...
    try:
        ...
        parser.exit(
            1,
            f"{_module_label(args.module_root)} 播种失败: {exc}\n",
        )
